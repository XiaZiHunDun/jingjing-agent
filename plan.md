...
| 答案来源引用 | ⚠️ 部分实现 | 有 source metadata，未在 UI 展示 |
...
| 钉钉机器人 | ❌ 未实现 | - |
| 飞书机器人 | ❌ 未实现 | - |
| API接口（FastAPI） | ⚠️ 部分实现 | 基础框架存在，需完善 |
...
| 任务 | 优先级 | 预计工作量 | 说明 |
|------|--------|-----------|------|
| 完善知识库管理界面 | P0 | 1天 | 添加文档列表、删除功能 |
| 答案来源引用展示 | P0 | 0.5天 | RAG 回答时显示引用来源 |
| FastAPI 后端完善 | P1 | 1-2天 | 完整 API 接口供外部调用 |
| 健康检查接口 | P1 | 0.5天 | `/health` 端点 |
| 时序数据库集成 | P2 | 2天 | InfluxDB 记录性能指标 |
| 基础监控 | P2 | 1天 | Sentry 或简单日志监控 |
...
- [ ] 完善知识库管理：添加文档列表页、删除功能
- [ ] RAG 答案来源引用：在回答中展示参考文档
- [ ] 测试 Docker 部署稳定性
